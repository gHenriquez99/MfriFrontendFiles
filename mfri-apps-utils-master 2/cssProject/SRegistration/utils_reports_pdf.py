...
from MBooks.utils import GetBooksAssignmentsForStudent, GetResourcesForCourse

from MOffices.utils import GetDefaultPrimaryPhoneNumberType, GetDefaultSecondaryPhoneNumberType
from MStaff.utils import split_name #, build_name_query, validate_ssn, parse_last_name_suffix, validate_last_name, 

from MSchedule.utils_reports import mfri_report_header_data

from SRegistration.utils import GetRegisteredStudents, GetPreRegisteredList #20170624    


from SRegistration.models import *


check_mark_file_name = u'%sgraphics/checkbox1.gif' % (settings.STATIC_ROOT) 
uncheck_mark_file_name = u'%sgraphics/checkbox0.gif' % (settings.STATIC_ROOT) 

check_mark_image = Image(check_mark_file_name, .15*inch, .15*inch) #20180607
uncheck_mark_image = Image(uncheck_mark_file_name, .15*inch, .15*inch) #20180607
...
def check_box_option(check_box_value=None, return_value_for_none=None, return_value_for_true=None, return_value_for_false=None): #, use_graphical_checks=True):


    if not return_value_for_none:
        return_value_for_none = uncheck_mark_image

    if not check_box_value:
        return return_value_for_none
    
    if not return_value_for_true:
        return_value_for_true = check_mark_image

    if not return_value_for_false:
        return_value_for_false = uncheck_mark_image
    
    if check_box_value:
        return return_value_for_true
    else:
        return return_value_for_false
...
def create_resource_assignment_report_pdf(request=None, scheduled_course=None, table_name=u'StudentRegistration'): #20170624
    ...
    if table_name == u'StudentRegistration':#20170624
        ...
        pre_registrations = GetPreRegisteredList(scheduled_course=scheduled_course, sort_by_name=True) #20170624    #20180307
        registered_student_list = pre_registrations.get('registered', []) #20170624 
        registered_student_list.extend(pre_registrations.get('wait', [])) #20180208
    ...
    lines_to_print_per_page = 10 / len(available_book_list)  #20170624
...
#                                            ('LINEBELOW', (0,-1), (-1,-1), 0.25, colors.black),
    ...
    for student_registration in registered_student_list:
        ...
        if student_record: 
            student_name = student_record.full_name #Paragraph(student_record.full_name, table_cell_stylesheet['Normal'])
        else: 
            student_name = u'%s (Not Seated)' % (student_registration.name)
        ...
        if assigned_book_list:
            for book_assignment in assigned_book_list:
                ...
                book_assignment_note = u''
                if book_assignment.note:
                    book_assignment_note = u'Unit: %s' % (book_assignment.note)

                if book_assignment.is_issued:
                    book_name = Paragraph(u'%s %s Amount Owed:$%.2f' % (book_assignment.book_description.short_name, book_assignment_note, book_assignment.price), table_cell_stylesheet['Normal']) #20210205
                elif book_assignment.is_declined:
                    book_name = Paragraph(u'%s %s' % (book_assignment.book_description.short_name, book_assignment_note), table_cell_stylesheet['Normal'])#20210205
                else:
                    book_name = Paragraph(u'%s %s New:$%.2f Used:$%.2f' % (book_assignment.book_description.short_name, book_assignment_note, book_assignment.book_description.new_price, book_assignment.book_description.used_price), table_cell_stylesheet['Normal'])#20210205
...
def GetRecipient(ssn=None, last_name=None, suffix=None, maiden_name=None):
    ...
    recipient = FindRecipient(ssn=ssn, last_name=last_name, suffix=suffix)

    if maiden_name and not recipient:
        recipient = FindRecipient(ssn=ssn, last_name=maiden_name, suffix=suffix)
