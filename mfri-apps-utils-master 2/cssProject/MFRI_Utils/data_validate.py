...
def validate_birth_date(birth_date=None, birth_date_required=True):
    
    if not birth_date:
        if birth_date_required:
            return {  
                     'StatusMessage': 'No Birthdate found',
                     'ResponseCode': -1,
                   }
        else:
            return {  
    ...
    if not validate_date(date_to_check=birth_date.strftime('%Y-%m-%d')):
       return {'ResponseCode': -2, 'StatusMessage': u'Invalid Date.'}

    return {'ResponseCode': 1, 'StatusMessage': u''}

def validate_student_address(address1=None, address2=None, city=None, state=None, post_code=None, country=None):

    address_validation_result = validate_address(address1=address1, 
                                                 address2=address2, 
                                                 city=city, 
                                                 state=state, 
                                                 post_code=post_code, 
                                                 country=country,
                                                 use_country=True, require_zip_plus_4_post_code=False, require_us_post_code=False)
...
#                                                 'country':{'description':u'', 'has_error': False, 'empty': False, 'invalid': False},
    ...
    if address_validation_result['address1']['invalid']:
        return_results['address1_error'] = address_validation_result['address1']['description'] 

    if address_validation_result['address2']['invalid']:
        return_results['address2_error'] = address_validation_result['address2']['description'] 

    if address_validation_result['city']['invalid']:
        return_results['city_error'] = address_validation_result['city']['description'] 

    if address_validation_result['state']['invalid']:
        return_results['state_error'] = address_validation_result['state']['description'] 

    if address_validation_result['post_code']['invalid']:
        return_results['postcode_error'] = address_validation_result['post_code']['description'] 

    if address_validation_result['country']['invalid']:
        return_results['country_error'] = address_validation_result['country']['description'] 

    return return_results


def validate_student_email_address(email_address=None, email_address_required=True):
    
    if not email_address:
        if email_address_required:
            return {  
    ...
    if not validate_email_address(email_address=email_address):
       return {'ResponseCode': -2, 'StatusMessage': u'Invalid Email Address.'}

    return {'ResponseCode': 1, 'StatusMessage': u''}

def validate_string(string_to_validate=None, is_required=False, string_type=u'name', min_Length=2, max_Length=80, validate_rule="^([a-zA-Z'/\- .]+)$", Debug=False): #20190206removed \""
    """
    string must be at least 2 characters long and less than 80 characters long.  Only alphanumeric characters, single quotes (') and dashes (-) allowed
    returns 1 if string passes and 0 if there is an error.
    """
    data_to_return = {  'StatusMessage': '',
                        'ResponseCode': 1,
                     }
    ...
    if is_required:
        if not string_to_validate:
            data_to_return['StatusMessage'] = u'No %s.' % (string_type)
            data_to_return['ResponseCode'] = -1
        
            return data_to_return
        
        if len(string_to_validate) == 0:
            data_to_return['StatusMessage'] = u'No %s.' % (string_type)
            data_to_return['ResponseCode'] = -2
    ...
    if len(string_to_validate) < min_Length: #non zero length, but still too short 
        data_to_return['StatusMessage'] = u'%s must be at least %d characters long.' % (string_type, min_Length)
        data_to_return['ResponseCode'] = -3
        
        return data_to_return

    if len(string_to_validate) > max_Length:
        data_to_return['StatusMessage'] = u'%s can not be longer than %d characters.' % (string_type, max_Length)
        data_to_return['ResponseCode'] = -4

        return data_to_return

    if not re.compile(validate_rule).search(string_to_validate):
        for char in string_to_validate:
            if not re.compile(validate_rule).search(char):
                data_to_return['StatusMessage'] = u'%s contains the invalid character "%s" (%s).' % (string_type, char, hex(ord(char))) #20190211
                data_to_return['ResponseCode'] = -5
                return data_to_return

        data_to_return['StatusMessage'] = u'%s contains an invalid character.' % (string_type)
        data_to_return['ResponseCode'] = -5
