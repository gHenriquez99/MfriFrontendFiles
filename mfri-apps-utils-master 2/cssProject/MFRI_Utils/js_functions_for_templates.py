...
def JSContentEditable(field_id_name=None):

    if not field_id_name:
        return ''
        
    return u'<script>var editable = document.getElementById("%s");addEvent(editable, "blur", function () {localStorage.setItem("contenteditable", this.innerHTML);document.designMode = "off";});addEvent(editable, "focus", function () { document.designMode = "on";});addEvent(document.getElementById("clear"), "click", function () { localStorage.clear();  window.location = window.location; });if (localStorage.getItem("contenteditable")) {editable.innerHTML = localStorage.getItem("contenteditable");}</script>' % (field_id_name)

def JSOnDoubleClickEvent(field_id_name=None, function_name=None, function_code=None): 
    ...
    js_section_start = u'<script type="text/javascript">' #$(document).ready(

    js_section = u'$(function() {$("#%s").bind("dblclick", function(event) { return %s(event); } );});' % (field_id_name, function_name)
...
def change_css_js(function_name='', check_box_id='', primary_id='', attribute_to_change='' , active_value='', secondary_id='', inactive_value='' ):
    return '''
function %s() {
    if ($("#%s").attr("checked"))
    {
...
''' % (function_name, check_box_id, primary_id, attribute_to_change, active_value, secondary_id, attribute_to_change, inactive_value, primary_id, attribute_to_change, inactive_value, secondary_id, attribute_to_change, active_value ) 


def change_show_label_js(function_name='mfri_primary_phone_number_help_update', show_check_box_id='id_show_mfri_primary_phone_number', help_label_id='mfri_primary_phone_number_help', check_box_true_label='(Show on public directory)', check_box_false_label='Do not show on public directory'):
    #    if ($("#%s").checked)
    return '''
function %s() {
    if ($("#%s").attr("checked"))
...
''' % (function_name, show_check_box_id, help_label_id, check_box_true_label, help_label_id, check_box_false_label)


def initialize_show_label_js(show_check_box_id='id_show_mfri_primary_phone_number', help_label_id='mfri_primary_phone_number_help', check_box_true_label='(Show on public directory)', check_box_false_label='Do not show on public directory'):
    return '''
if ($("#%s").attr("checked"))
{
    $("#%s").text("%s");
...
''' % (show_check_box_id, help_label_id, check_box_true_label, help_label_id, check_box_false_label)


def disable_submit_buttons_js(form_id='profile_contact_form'):
    return '''
...
def disable_input_onsubmit_js(js_function_name='disable_input_buttons', save_button_id='id_save_form', cancel_button_id='id_cancel_form'):
    return '''
function %s(event) {
   $submitcounter++;
   if ($submitcounter > 1)
   {
     $("#%s").val("Action pending...");
     $("#%s").val("");
     $(":input").attr("disabled", "disabled");
     $("a").click(function(e) { e.preventDefault(); });
     event.preventDefault();
...
''' % (js_function_name, save_button_id, cancel_button_id)
